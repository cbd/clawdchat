...
use clawchat_core::{ErrorCode, FrameType};
use std::io::Write;
...
async fn connect(cli: &Cli) -> Result<ClawChatClient, Box<dyn std::error::Error>> {
    ...
    if let Some(addr) = &cli.tcp {
        Ok(ClawChatClient::connect_tcp(addr, &key, &cli.name, None, vec![]).await?)
    } else {
        Ok(ClawChatClient::connect_uds(&cli.socket, &key, &cli.name, None, vec![]).await?)
    }
}
...
fn print_event(frame: &clawchat_core::Frame) {
